...
function ignite_plugin_update_row( $file, $plugin_data ) {
	...
	if ( count( $ignitewoo_updater_activated ) > 0 )
	...
	if ( ! class_exists( 'IgniteWoo_Updater' ) ) {
		...
		foreach ( $plugins as $plugin ) {
			...
			if ( strpos( $plugin, 'ignitewoo-updater.php' ) !== false ) {
				...
				_e( sprintf( '<a href="%s">Active the IgniteWoo Updater</a> so to receive updates and support', $url ), 'ignitewoo' );
	...
	} else if ( empty( $ignitewoo_updater_activated ) || !$ignitewoo_updater_activated || !$got_it ) {
