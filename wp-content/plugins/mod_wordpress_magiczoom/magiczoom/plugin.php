...
function WordPress_MagicZoom_send_stat($action = '') {
    ...
    if('working' == 'v5.10.5' || 'working' == 'v4.0.25') {
        return;
    }
    ...
    $path = "api/stat/?action={$action}&tool_name=magiczoom&license=trial&tool_version=v4.0.25&module_version=v5.10.5&platform_name=wordpress&platform_version={$platformVersion}&url={$url}";
...
function WordPress_MagicZoom_LoadScroll($tool) {
...
function  magictoolbox_WordPress_MagicZoom_init() {
    ...
    if (!file_exists(dirname(__FILE__) . '/core/magiczoom.js')) {
        $jsContents = file_get_contents('http://www.magictoolbox.com/static/magiczoom/trial/magiczoom.js');
        if (!empty($jsContents) && preg_match('/\/\*.*?\\\*/is',$jsContents)){
            if ( !is_writable(dirname(__FILE__) . '/core/')) {
                $error_message = 'The '.substr(dirname(__FILE__),strpos(dirname(__FILE__),'wp-content')).'/core/magiczoom.js file is missing. Please re-uplaod it.';
            }
            file_put_contents(dirname(__FILE__) . '/core/magiczoom.js', $jsContents);
            chmod(dirname(__FILE__) . '/core/magiczoom.js', 0777);
        } else {
            $error_message = 'The '.substr(dirname(__FILE__),strpos(dirname(__FILE__),'wp-content')).'/core/magiczoom.js file is missing. Please re-uplaod it.';
        }
    }
...
function WordPress_MagicZoom_get_prepared_selectors () {
    $selectors = array();
    $plugin = $GLOBALS['magictoolbox']['WordPressMagicZoom'];
    $attachments = WordPress_MagicZoom_get_post_attachments();
    ...
    return array_reverse($selectors);
}
