...
class WC_Shipping_UPS extends WC_Shipping_Method {
    ...
    private function init() {
		...
		$this->origin_country   = isset( $this->settings['origin_country'] ) ? $this->settings['origin_country'] : WC_Countries::get_base_country();
		...
		if ( $woocommerce->countries->get_base_country() == 'PL' ) {
	...
	private function environment_check() {
		...
		if ( ! $this->origin_country && $this->enabled == 'yes' ) {
			$error_message .= '<p>' . __( 'UPS is enabled, but the origin country has not been set.', 'wc_ups' ) . '</p>';
		}
    ...
    public function init_form_fields() {
    	...
    	$this->form_fields  = array(
		    ...
		    'shipper_number'      => array(
				'title'           => __( 'UPS Shipper Number', 'wc_ups' ),
				'type'            => 'text',
				'description'     => __( 'Obtained from UPS after getting an account.', 'wc_ups' ),
				'default'         => '',
		    ),
			...
			'origin_country'        => array(
				'title'           => __( 'Origin Country', 'wc_ups' ),
				'type'            => 'select',
				'class'			  => 'chosen_select',
				'css'			  => 'width: 250px;',
				'description'     => __( 'Enter the country for the <strong>sender</strong>.', 'wc_ups' ),
    ...
    public function calculate_shipping( $package ) {
    	...
    	if ( $package_requests ) {
			...
			$rate_requests = $this->get_rate_requests( $package_requests );
	 ...
	 *
	 */
	private function get_rate_requests( $package_requests ) {
		global $woocommerce;
		...
		foreach ( $this->custom_services as $code => $params ) {		
			if ( 1 == $params['enabled'] ) {
	    		...
	    		$request .= "		<ShipTo>" . "\n";
	    		$request .= "			<Address>" . "\n";
	    		$request .= "				<StateProvinceCode>" . $customer->get_shipping_state() . "</StateProvinceCode>" . "\n";
	    		$request .= "				<PostalCode>" . $customer->get_shipping_postcode() . "</PostalCode>" . "\n";
	    		$request .= "				<CountryCode>" . $customer->get_shipping_country() . "</CountryCode>" . "\n";
	    		if ( $this->residential ) {
	    		...
	    		$request .= "			<NegotiatedRatesIndicator></NegotiatedRatesIndicator>" . "\n";
	    		$request .= "		</ShipmentRatingOptions>" . "\n";
				}
	    		$request .= "	</Shipment>" . "\n";
	    		$request .= "</RatingServiceSelectionRequest>" . "\n"; 
    ...
    private function per_item_shipping( $package ) {
    	...
    	foreach ( $package['contents'] as $item_id => $values ) {
    		...
    		if ( $values['data']->length && $values['data']->height && $values['data']->width ) {
				...
				$dimensions = array( number_format( woocommerce_get_dimension( $values['data']->length, 'in' ), 2, '.', ''), 
									 number_format( woocommerce_get_dimension( $values['data']->height, 'in' ), 2, '.', ''),  
									 number_format( woocommerce_get_dimension( $values['data']->width, 'in' ), 2, '.', '') );
				sort( $dimensions );

			} else {
	    		if ( $this->debug )
	    			$woocommerce->add_error( sprintf( __( 'Individual packing method is enabled and product #%d is missing dimensions. Aborting.', 'wc_ups' ), $ctr ) );
	    		return;
			}
    ...
    private function box_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $values['data']->length && $values['data']->height && $values['data']->width && $values['data']->weight ) {
	    		...
	    		if ( $this->debug )
					$woocommerce->add_error( sprintf( __( 'Product #%s is missing dimensions. Aborting.', 'wc_ups' ), $item_id ) );
				return;
			}
		}
